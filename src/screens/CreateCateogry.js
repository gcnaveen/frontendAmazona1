...
const [categoryInputs,setCategoryInputs]=useState({name:'',slug:'',subCategory:[{name:'',slug:''}]})
    ...
    const addSubCategory = () => {
        setCategoryInputs({ ...categoryInputs, subCategory: [...categoryInputs.subCategory, { name: '', slug: '' }] })
    ...
    const handleSubmit = (e) => {
       ...
       axios.post('/api/products/newCategory', categoryInputs).then(res => {
    ...
    const deleteHandler = async (cat) => {
        ...
        try{
        let {data}=  await axios.delete(`/api/products/deleteCategory/${catID}`)
   ...
   toast.error(getError(error))
    ...
    const submitUpdateForm =  async  (e) => {
        ...
        const categoryID=categoryInputs._id
             ...
             try {
         const { data } =await axios.put(`/api/products/updateCategory/${categoryID}`,categoryInputs)
   ...
   toast.error(getError(error))
    ...
    return <div className='container'>
        ...
        {!isUpdate ? <Button type="submit" style={{width:'220px'}} onClick={()=>setUpdate(true)} >
              Update Existing Category
        </Button> :   !isUpdateForm.status &&  <Button type="submit" style={{width:'80px'}} onClick={()=>setUpdate(false)} >
              Create 
            </Button>}
       </div>
        {isUpdate ? <>
            ...
            {!isUpdateForm.status && <table className="table">
            <thead>
              <tr>
                <th>ID</th>
                <th>NAME</th>
                ...
                <form onSubmit={submitUpdateForm}>
                    ...
                    <div className='row'>
                        ...
                        <div className='col-md-6 col-sm-12'>
                            <label>Name</label>
                            <input onChange={handleInputFields} name='name'   value={categoryInputs.name} className='form-control' />
                        </div>
                        <div className='col-md-6 col-sm-12'>
                            <label>Slug</label>
                            <input onChange={handleInputFields} name='slug' value={categoryInputs.slug} className='form-control' />
                        </div>
                    </div>
                    ...
                    {categoryInputs.subCategory.map((subCat, i) => {
                        return <div className='row' key={i}>
                            <div className='col-md-5 col-sm-12'>
                                <label>Name</label>
                                <input onChange={(e) => handleSubCat(e, i)} value={subCat.name}  name='name' className='form-control' />
                            </div>
                            <div className='col-md-5 col-sm-12'>
                                <label>Slug</label>
                                <input onChange={(e) => handleSubCat(e, i)} value={subCat.slug} name='slug' className='form-control' />
                            </div>
            ...
            <form onSubmit={handleSubmit} >
                <div>
                    ...
                    <div className='row'>
                        ...
                        <div className='col-md-6 col-sm-12'>
                            <label>Name</label>
                            <input onChange={handleInputFields} name='name' className='form-control' />
                        </div>
                        <div className='col-md-6 col-sm-12'>
                            <label>Slug</label>
                            <input onChange={handleInputFields} name='slug' className='form-control' />
                        </div>
                    </div>
